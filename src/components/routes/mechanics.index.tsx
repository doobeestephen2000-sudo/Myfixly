...
import { NIGERIA_STATES, GENERATOR_BRANDS, SERVICES, TRADES, MECHANIC_PUBLIC_COLUMNS } from "@/lib/constants";
...
function MechanicsList() {
  ...
  const { data: mechanics = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters.brand) q = q.contains("brands", [filters.brand]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="sticky top-16 z-30 -mx-4 mb-6 border-y border-border/60 bg-background/95 px-4 py-3 backdrop-blur sm:mx-0 sm:rounded-2xl sm:border sm:px-4 sm:shadow-card">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-0 flex-1">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input value={filters.q} onChange={(e) => setFilters({ ...filters, q: e.target.value })} placeholder="Search name, brand, city..." className="pl-9" />
          </div>
          <select value={filters.trade} onChange={(e) => setFilters({ ...filters, trade: e.target.value })} className="h-10 rounded-md border border-input bg-background px-3 text-sm">
        ...
        {showFilters && (
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <select value={filters.brand} onChange={(e) => setFilters({ ...filters, brand: e.target.value })} className="h-10 rounded-md border border-input bg-background px-3 text-sm">
              <option value="">All brands / specialties</option>
              {GENERATOR_BRANDS.map((b) => <option key={b} value={b}>{b}</option>)}
            </select>
            ...
            {activeCount > 0 && (
              <Button variant="ghost" onClick={() => setFilters({ q: "", trade: "", state: "", city: "", brand: "", service: "" })} className="justify-start text-muted-foreground">
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border bg-card p-16 text-center">
          <h3 className="font-display text-xl font-semibold">No pros found</h3>
          <p className="mt-2 text-sm text-muted-foreground">Try clearing filters or searching another area.</p>
          <Button asChild variant="outline" className="mt-4"><Link to="/mechanics">Reset search</Link></Button>
        </div>
      ) : (
        <>
