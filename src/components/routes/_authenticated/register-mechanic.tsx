...
import { NIGERIA_STATES, GENERATOR_BRANDS, SERVICES, TRADES, formatNaira } from "@/lib/constants";
...
function RegisterMechanic() {
  ...
  const [f, setF] = useState({
    ...
    brands: (existing?.brands ?? []) as string[],
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold">Complete your mechanic registration</h1>
        ...
        <form onSubmit={saveProfile} className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <Label>Brands / specialties (optional)</Label>
                <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {GENERATOR_BRANDS.map((b) => (
                    <label key={b} className="flex items-center gap-2 rounded-md border border-border p-2 text-sm">
                      <Checkbox checked={f.brands.includes(b)} onCheckedChange={(v) => setF((prev) => ({ ...prev, brands: v ? [...prev.brands, b] : prev.brands.filter((x) => x !== b) }))} />
                      {b}
                    </label>
                  ))}
                </div>
              ...
              <Field label="Upload ID (for verification)">
                <Input type="file" accept="image/*,.pdf" onChange={async (e) => {
                  const file = e.target.files?.[0]; if (!file) return;
                  const url = await uploadFile("mechanic-ids", file);
                  if (url) { setF((prev) => ({ ...prev, id_document_url: url })); toast.success("ID uploaded"); }
                }} />
                {f.id_document_url && <p className="mt-1 text-xs text-success">ID uploaded ✓</p>}
              </Field>
            </CardContent>
          </Card>
